...
function runGrantFlow(options, done, failed) {
    if (options.secret) {
        sendAuthViaHeader(options, done, (error) => sendAuthViaParams(options, done, failed, error));
    } else {
        sendAuthViaParams(options, done, (error) => sendAuthViaHeader(options, done, failed, error));
    }
}

function sendAuthViaHeader(options, done, failed, abortError) {
    const request = Object.assign({}, options.request);
    request.auth = options.client + ':' + options.secret;
    sendRequest(request, options.payload, done, failed, abortError);
}

function sendAuthViaParams(options, done, failed, abortError) {
    const request = Object.assign({}, options.request);
    const secret = options.secret ? `&client_secret=${options.secret}` : '';
    const payload = `${options.payload}&client_id=${options.client}${secret}`;
    sendRequest(request, payload, done, failed, abortError);
}

function sendRequest(options, payload, done, failed, abortError) {
    ...
    request
        ...
        .on('error', (error) => {
            switch(error.code) {
                case 'ECONNRESET':
                case 'ECONNABORTED':
                    error = abortError ? abortError : ErrMsg(EC.OAUTH_RESPONSE_ERROR, error.message);
            }
            failed(error);
        })
        .on('aborted', (error) => {
